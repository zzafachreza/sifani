        ...
        launchImageLibrary(options, (response) => {
            if (response.didCancel) {
                ...
                if (image.uri && image.fileSize > 0) {
                    ...
                    setKirim({ ...kirim, suami_foto: base64Image });
                    setfoto(image.uri);
                } else {
    ...
    return (
        <SafeAreaView style={{ flex: 1, backgroundColor: colors.white, padding: 10 }}>
            <ScrollView showsVerticalScrollIndicator={false}>
                <MyInput
                    value={kirim.suami_nama}
                    label="Nama"
                    onChangeText={(x) => setKirim({ ...kirim, suami_nama: x })}
                    iconname="person"
                    placeholder="Masukkan nama"
                />
                <MyGap jarak={10} />
                <MyInput
                    value={kirim.suami_tempat_lahir}
                    label="Tempat Lahir"
                    onChangeText={(x) => setKirim({ ...kirim, suami_tempat_lahir: x })}
                    iconname="person"
                ...
                <MyGap jarak={10} />
                <MyInput
                    value={kirim.suami_pekerjaan}
                    label="Pekerjaan"
                    onChangeText={(x) => setKirim({ ...kirim, suami_pekerjaan: x })}
                    iconname="clipboard"
                ...
                <MyInput
                    ...
                    maxLength={15}
                    value={kirim.suami_telepon}
                    label="Telepon / Nomor HP"
                    onChangeText={(x) => setKirim({ ...kirim, suami_telepon: x })}
                    iconname="call"
                    placeholder="Masukkan telepon"
                />
                <MyGap jarak={10} />
                <MyInput
                    value={kirim.suami_alamat}
                    label="Alamat"
                    onChangeText={(x) => setKirim({ ...kirim, suami_alamat: x })}
                    iconname="home"
